...
(function () {
  // Bridge to the VS Code extension host when running inside a VS Code webview.
  // The Pilot (Electron) host exposes no acquireVsCodeApi, so guard the call and
  // no-op there. Used to surface viewer lifecycle to the smoke check (issue 084)
  const vscodeApi =
  ...
  window.addEventListener('load', async function () {
    ...
    PDFViewerApplication.initializedPromise.then(() => {
      // Signal a successful end-to-end load to the extension host so the smoke
      // check (issue 084) can assert the viewer actually rendered a document
  ...
  function reportError(kind, detail) {
    // Let the extension host observe the failure so the smoke check (issue 084)
    // fails instead of silently packaging a broken viewer
    postToHost({ type: 'pdf-viewer-error', kind: kind, detail: detail })
    try {
